...
public class SimpleItemComponentBuilder<B extends SimpleItemComponentBuilder<B>> extends AbstractGuiComponentBuilder<B> {
    ...
    protected @NotNull ItemStack getItemStack() {
        if (material != null) return ItemUtils.createItem(material, name, 1, lore);
        return textureId != null ? ItemUtils.createHead(textureId, name, 1, lore) : owner != null ? ItemUtils.createSkull(Bukkit.getOfflinePlayer(owner), 1, name, lore) : ItemUtils.createItem(Material.PLAYER_HEAD, name, 1, lore);
    }
